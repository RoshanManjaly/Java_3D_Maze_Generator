 ...
 * 
 * 
 * The maze is built with a randomized version of Prim's algorithm. This means a
 * spanning tree is expanded into a set of cells by removing walls from the
...
public class MazeBuilderPrim extends MazeBuilder implements Runnable {
	...
	/**
	 * This method generates pathways into the maze by using Prim's algorithm to
	 * generate a spanning tree for an undirected graph. The cells are the nodes of
	 * the graph and the spanning tree. An edge represents that one can move from
	 * one cell to an adjacent cell. So an edge implies that its nodes are adjacent
	 * cells in the maze and that there is no wall separating these cells in the
	 * maze.
	 */
	@Override
	protected void generatePathways() {
		...
		// those walls lead to adjacent cells that are not part of the spanning tree
		// yet.
		...
		while (!candidates.isEmpty()) {
			...
			// check if wall leads to a new cell that is not connected to the spanning tree
			// yet
			if (cells.canGo(curWall)) {
				...
				updateListOfWalls(x, y, candidates); // checks to see if it has walls to new cells, if it does it adds
														// them to the list
				// note that each wall can get added at most once. This is important for
	...
	/**
	 * Updates a list of all walls that could be removed from the maze based on
	 * walls towards new cells
	 * 
	 * @param x
	 * @param y
	 */
