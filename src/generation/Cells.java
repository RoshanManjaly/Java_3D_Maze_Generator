...
/**
 * This class encapsulates all access to a grid of cells. Each cell encodes
 * whether walls or borders/bounds to rooms or to the outer border of the maze
 * exist. The internal two-dimensional array matches with a grid of cells as
 * follows: cells[0,y] form the left border, hence there is a wall on left.
 ...
 * 
 * Note that for a calculated maze, at least one cell on the border will have a
 * missing wall for an exit somewhere.
 * 
 * Walls and borders are separated concepts. A border is not removed by the maze
 * generation procedure. It is used to mark the outside border of the maze as
 * well as those walls of internal rooms that should remain in place. One can
 * think of the border attribute as a little sticker on a wall that says "leave
 * this wall in place". Walls can be taken down by the maze generation
 * procedure.
 * 
 * The internal encoding of walls for each cell into a single integer per cell
 * is performed with bit operations (&,|) and thus error prone. An encapsulation
 * within this class localizes all bit operations for this encoding.
 * 
 ...
 * modify and use code for teaching purposes. The class resulted from
 * refactoring the int[][] cells area in the original Maze and Mazebuilder
 * classes into a class of its own. Refactored by Peter Kemper
 */
public class Cells {

	// Integer constants to encode 4 possible walls for a single cell (CW = cell
	// wall) on top, bottom, left, right in a single byte of an integer
	...
	public static final int CW_VISITED = 16; // 2^4 is used as a flag to indicate if a cell is new (1) or if it has been
												// visited before (0)
	public static final int CW_ALL = CW_TOP | CW_BOT | CW_LEFT | CW_RIGHT; // constant to simplify checking if all walls
																			// are present
	// Integer constants to encode 4 possible sides that touch a border (or bound)
	// a separate encoding of borders allows for flexible layouts (not just
	// rectangles)
	// Note: encoding matches the wall encoding with respect to directions such that
	// same encoding applies plus a shift
	public static final int CW_BOUND_SHIFT = 5; // used to shift encoding from dirsx, dirsy below from wall range to
												// bound range
	...
	// public static final int CW_ALL_BOUNDS =
	// CW_TOP_BOUND|CW_BOT_BOUND|CW_LEFT_BOUND|CW_RIGHT_BOUND; // constant to
	// simplify check if all all bounds are present
	public static final int CW_IN_ROOM = 512; // 2^9
	// we put all encodings into a single array such that it is easier to iterate
	// over the array
	// note that the numerical values are used for bitwise calculations so a
	// refactoring with other values in an enumeration can break the code
	// Directions:
	...
	// changed to public for BasicRobot
	public int width;
	public int height;
	...
	// cells[i][j] is not intuitive in terms of rows and column indices in matrix
	// notation
	...
	/**
	 * Constructor that dimensions and initializes cells with the values from the
	 * given matrix.
	 * 
	 * @param input
	 *            provides input data to copy cell content from
	...
	public Cells(int[][] input) {
		...
		// create the maze dimensions
		cells = new int[width][];
		for (int i = 0; i < width; i++)
			cells[i] = input[i].clone();
		/*
		 * Basic version this(input.length, input[0].length); for (int i = 0; i < width;
		 * i++) { for (int j = 0; j < height; j++) { cells[i][j] = input[i][j]; } }
		 */
	}

	/**
	 * Initialize maze such that all cells have not been visited, all walls inside
	 * the maze are up, and borders form a rectangle on the outside of the maze.
	 */
	...
	/**
	 * Hashcode method is not implemented as it is not needed here. Dummy method to
	 * recognize common pitfall from overriding equals() but not hashcode() method.
	 */
	@Override
	public int hashCode() {
	 ...
	 * @return value with internal encoding of walls and other attributes for the
	...
	/**
	 * checks if cell (x,y) and adjacent neighbor in the given direction are not
	 * separated by a border and the neighbor has not been visited before.
	 * 
	 * @param wall
	 *            provides (x,y) coordinates for cell and the direction
	 * @precondition borders limit the outside of the maze area
	 * @precondition 0 <= x < width, 0 <= y < height
	 * @return true if neighbor in the given direction is new and wall can be taken
	...
	public boolean canGo(Wall wall) {
		...
		int dx = d[0];
		int dy = d[1];
		// borders limit rooms (but for potential doors) and the outside limit of the
		// maze
	...
	/**
	 * Tells if the given position is visited for the first time. This is true after
	 * cells.initialize() and before setCellAsVisited(x,y).
	 * 
	 * @param x
	 *            coordinate of cell
	 * @param y
	...
	/**
	 * Establish exit position by breaking down wall to outside area. If (x,y) is
	 * not located next to an outside wall, the method no effect.
	 * 
	 * @param x
	 * @param y
	 * @precondition 0 <= x < width, 0 <= y < height
	...
	public void setExitPosition(int x, int y) {
		...
		// System.out.println("exit position set to zero: " + remotex + " " + remotey +
		// " " + bit + ":" + cells.hasMaskedBitsFalse(remotex, remotey, bit)
		// + ", Corner case: " + ((0 == remotex && 0 == remotey) || (0 == remotex &&
		// height-1 == remotey) || (width-1 == remotex && 0 == remotey) || (width-1 ==
		// remotex && height-1 == remotey)));
	}
	 ...
	 *            coordinate of cell
	 * @precondition 0 <= x < width, 0 <= y < height
	 * @return true if position is on the border and there is no wall to the
	...
	/**
	 * Tells if the given position is inside a room. This is false after
	 * cells.initialize() and before calling setInRoomToOne() or markAreaAsRoom().
	 * 
	 * @param x
	 *            coordinate of cell
	 * @param y
	 *            coordinate of cell
	 * @precondition 0 <= x < width, 0 <= y < height
	 * @return true if (x,y) position resides in an area marked as a room before,
	 *         false otherwise
	 */
	...
	/**
	 * Checks if there is a cell in the given area that belongs to a room. The first
	 * corner (rx,ry) is at the upper left position, the second corner (rxl,ryl) is
	 * at the lower right position.
	 * 
	 * @param rx
	 *            1st corner, x coordinate
	 * @param ry
	 *            1st corner, y coordinate
	 * @param rxl
	 *            2nd corner, x coordinate
	 * @param ryl
	 *            2nd corner, y coordinate
	 * @precondition 0 <= rx <= rxl < width, 0 <= ry <= ryl < height
	 * @return true if area contains a cell that is already in a room or if it is
	 *         too close to the border, false otherwise
	 */
	...
	/**
	 * Marks a given area as a room on the maze and positions up to five doors
	 * randomly. The first corner is at the upper left position, the second corner
	 * is at the lower right position. Assumes that given area is located on the map
	 * and does not intersect with any existing room. The walls of a room are
	 * declared as borders to prevent the generation mechanism from tearing them
	 * down. Of course there must be a few segments where doors can be created so
	 * the border protection is removed there.
	 * 
	 * @param rw
	 *            room width
	 * @param rh
	...
	/**
	 * Sets bounds on the perimeter of an internal area with bound and wall to
	 * enclose area. Bounds and walls are added from both directions. with upper
	 * left corner (rx,ry), lower right corner (rxl,ryl).
	 * 
	 * @param rx
	 * @param ry
	 * @param rxl
	 ...
	 *            coordinate of cell
	 * @param dx
	 *            direction x, in { -1, 0, 1} obtained from dirsx[]
	 * @param dy
	 *            direction y, in { -1, 0, 1} obtained from dirsy[]
	 * @precondition 0 <= x < width, 0 <= y < height
	 */
	...
	/**
	 * Calculates the bitmask for a bound that corresponds to a given bitmask for a
	 * direction CW_LEFT, CW_RIGHT, etc
	 * 
	 * @param bit
	 *            in {CW_LEFT, CW_RIGHT, CW_TOP, CW_BOT}
	...
	/**
	 * Delete a border/bound between to adjacent cells (x,y) and (x+dx,y+dy). Only
	 * used in markAreaAsRoom.
	 * 
	 * @param x
	 *            coordinate of cell
	 * @param y
	...
	/**
	 * Add a wall and a border/bound between to adjacent cells (x,y) and
	 * (x+dx,y+dy). Only used in markAreaAsRoom.
	 * 
	 * @param x
	 *            coordinate of cell
	 * @param y
	 ...
	 *            coordinate of cell
	 * @param direction
	 *            from given cell to neighbor cell
	 * @param internalWall
	 *            denotes if wall is added on both cells (true) or just for the
	 *            given cell (false)
	 */
	...
	/**
	 * Delete a wall between to adjacent cells (x,y) and (x+dx,y+dy). Deleting the
	 * wall at (x,y) implies that also a wall in opposite direction at (x+dx,y+dy)
	 * gets deleted.
	 * 
	 * @param wall
	 *            provides (x,y) coordinate of cell and direction
	 */
	public void deleteWall(Wall wall) {
		...
		/////////////////// THE FOLLOWING 2 LINES ARE USED FOR GRADING PROJECT 2, DO NOT
		/////////////////// ALTER OR DELETE /////////////////
	...
	/**
	 * Add walls in either north or west direction for the given segment. Segment
	 * coordinates need to be rescaled by map_unit to translate into cell
	 * coordinates. Method is only used to track seen cells in the FirstPersonDrawer
	 * for the MapDrawer. So achieved properties for cells object differ than for
	 * other methods used to represent the maze.
	 * 
	 * @param seg
	 *            gives the segment whose walls need to be added
	 * @param map_unit
	 *            gives the scaling factor to obtain the cell coordinates
	 */
	public void addWallsForSegment(Seg seg, int map_unit) {
		...
		// final int sdx = seg.getExtensionX() / map_unit; // constant, only set once
		// here
		// final int sdy = seg.getExtensionY() / map_unit; // constant, only set once
		// here
		...
		assert (sdsx != 0 && sdsy == 0)
				|| (sdsx == 0 && sdsy != 0) : "Segment needs to extend into exactly one direction";
		// note: step 2 possibly changes range for sx, sy such that end point may be at
		// -1
	...
	//////////////////// ///////////////////////
	/**
	 * Tells if the given position has a wall in the given direction. This is true
	 * after cells.initialize() and before deleting this wall. A wall can be deleted
	 * by directly calling deleteWall() or by removing all walls within a room,
	...
	/**
	 * Tells if the given position has no wall in the given direction. This is true
	 * after cells.initialize() and before deleting this wall. A wall can be deleted
	 * by directly calling deleteWall() or by removing all walls within a room,
	 * method markAreaAsRoom().
	 * 
	...
	/**
	 * Gives the matching bit encoded value, i.e., the matching CW_TOP, BOT, LEFT,
	 * RIGHT constants in Constants.java for the current direction. TOP is matched
	 * with North, Right is matched with East.
	 * 
	 * @return the matching integer value (CW_ constant)
	 */
	protected int getCWConstantForDirection(CardinalDirection dir) {
		/*
		 * compare with definition for consistency static final int CW_TOP = 1; // 2^0
		 * static final int CW_BOT = 2; // 2^1 static final int CW_LEFT = 4; // 2^2
	...
	/**
	 * Inner class to provide an iterator that delivers tuples of [start,end]
	 * indices for the begin and end of a continuous sequence of walls.
	 * 
	 * The class resulted from refactoring the BSPBuilder class. In order to cater
	 * to the existing needs, the delivered data is quite particular.
	 * 
	 * A sequence of walls can extend in two directions (horizontal, i.e. along x
	 * axis or vertical, i.e., along the y axis) and for each direction walls can be
	 * on one of the two sides. Vertical sequence: along east or west side of a cell
	 * Horizontal sequence: along north or south side of a cell
	 * 
	 * The iterator goes through increasing values of x if horizontal (y if
	 * vertical).
	 * 
	 * While the start index refers to a cell that has a wall and where the sequence
	 * starts (i.e. the cell before has no wall), the end index refers to a cell
	 * where the sequence of walls has ended before (i.e. the cell before has a wall
	 * but the end index cell has no wall).
	 * 
	 * @author pk
	 *
	 */
	private class SequenceIterator implements Iterator<int[]> {
		...
		/**
		 * Constructor to obtain an iterator that provides [start,end] value pairs for
		 * either x or y coordinates of cells that have a continuous sequence of walls
		 * on the given side. Note that start is a cell with a wall, end is a cell that
		 * is the first with no wall after the end of the sequence. If the given
		 * direction is North or South, the method looks for a horizontal sequence of
		 * walls (increments x, so it walks east). If the given direction is East or
		 * West, the method looks for a vertical sequence of walls (increments y).
		 * 
		 * @param x
		 *            x-coordinate of first cell to consider
		 * @param y
		 *            y-coordinate of first cell to consider
		...
		/**
		 * Finds the next sequence of walls for the stored direction that starts from
		 * the current (startX,startY) position. Method updates either startX or startY.
		 * If the given direction is North or South, the method looks for a horizontal
		 * sequence of walls (increments x). If the given direction is East or West, the
		 * method looks for a vertical sequence of walls (increments y).
		 * 
		 * @return [start,end] pair of either x or y coordinates where start has a wall
		 *         for the given direction and end is the first cell beyond the sequence
	...
	////////////////// //////////////////////////////////////////
	// long term goal is to make all of these methods private to encapsulate the
	////////////////// encoding ///////////////////////
	/**
	 ...
	 *            coordinate of cell
	 * @param cw_bit
	 *            like CW_LEFT, CW_RIGHT, CW_TOP, CW_BOTTOM
	 * @precondition 0 <= x < width, 0 <= y < height
	 */
	...
	private int getBit(int dx, int dy) {
		/*
		 * int bit = 0; switch (dx + dy * 2) { case 1: bit = Constants.CW_RIGHT; break;
		 * // dx=1, dy=0 case -1: bit = Constants.CW_LEFT; break; // dx=-1, dy=0 case 2:
		 * bit = Constants.CW_BOT; break; // dx=0, dy=1 case -2: bit = Constants.CW_TOP;
		 * break; // dx=0, dy=-1 default: dbg("getBit problem "+dx+" "+dy); break; }
		 * return bit;
		 */
	...
	/**
	 * Methods dumps internal data into a string, intended usage is for debugging
	 * purposes. Maze is represent as a matrix of integer values.
	 */
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////// ////////////////////////////////////////////////////
	// flag to trigger that a log is constructed that lists the sequence of walls
	///////////////////////////////////////////////////////////////////////////////////////////////////////////// that
	///////////////////////////////////////////////////////////////////////////////////////////////////////////// are
	///////////////////////////////////////////////////////////////////////////////////////////////////////////// deleted
	...
	/**
	 * Append wall information to logging data. Currently used to log the sequence
	 * of walls that are deleted in the maze generation phase
	 * 
	 * @param x
	 ...
	 * @param dx
	 *            direction, x coordinate, -1 <= dx <= 1
	 * @param dy
	 *            direction, y coordinate, -1 <= dy <= 1
	 */
