...
/**
 * This class implements the generation of pseudo random numbers with a single
 * instance of a random number generator. It is possible to set the seed of the
 * generator by calling setSeed before the internal random number generator is
 * initialized. This happens when getRandom is called for the first time. Any
 * subsequent call to setSeed resets the seed of the existing generator. Design
...
public final class SingleRandom {
	/**
	 * internal internal random number generator. This should NOT be made
	 * accessible.
	 */
	...
	/**
	 * optional: a seed value can be set to be able to reproduce a sequence of
	 * random numbers.
	 */
	...
	/**
	 * lock to make the private constructor thread safe, one could have used the
	 * class object as well
	 */
	private static final Object LOCK = new Object();

	/**
	 * The constructor for a class that allows for a single instance must be
	 * private.
	 */
	...
	public int nextIntWithinInterval(final int lowerBound, final int upperBound) {
		...
		// say d = upperBound-lowerBound, then we want a result lowerBound +
		// uniform(0,d) and uniform(0,d) is delivered by nextInt(d+1)
	 ...
	 * 
	 * @param seed
	 *            is the seed value for the random number generator
	 */
