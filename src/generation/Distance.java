...
/**
 * This class has the responsibility to provide the distance for each cell to
 * the exit of a maze. It encapsulates the distance matrix. All methods assume
 * that given (x,y) coordinates are with its legal range
 * [0,width-1],[0,height-1] such that no additional parameter checks are
 * performed. This is reasonable as this class is used only internally to the
 * package.
 * 
...
public class Distance {
	// use of indices between Distance and Cells internal 2D matrices is consistent
	// such that
	...
	/**
	 * Gets access to a width x height array of distances. Warning, this exposes the
	 * internal attribute for read access only. Do not modify entries of the
	 * returned array.
	 * 
	...
	/**
	 * Sets the internal attribute to the given parameter value. Warning, the array
	 * is not copied but directly shared with the environment that provides it! This
	 * can have undesired side effects. Handle with care!
	 * 
	 * @param array
	...
	/**
	 * Finds the most remote point in the maze somewhere on the border. Requires
	 * that distances have been computed beforehand.
	 * 
	...
	/**
	 * Get the position of the entry with the smallest value. If the values of the
	 * distance matrix are accurate, the result is the exit position.
	 * 
	...
	/**
	 * Computes distances to the given position (ax,ay) for all cells in array
	 * dists.
	 * 
	 * @param cells
	 *            provide information on walls between positions
	 * @param ax,
	...
	private void computeDists(Cells cells, int ax, int ay) {
		...
		// MEMO: there are likely to be much smarter ways to distribute distances in a
		// breadth first manner...
		// why not push identified cells with infinite distance on a "work to do" heap
		// : limit iterations to upperbound which is #cells == width * height
		...
		// : do a final checkup for enclosed areas left over with Max Int
		do {
		...
		assert (0 == countInfinity()) : "All positions should have a finite distance to the exit";
		// the computation is not good enough to have truly minimum values
		saturateDistances(cells);
	}

	/**
	 * Follows a path as far as possible to push small distance values into the maze
	 * 
	 * @param cells
	 *            needed for checking walls, read only
	 * @param sx
	 *            starting position
	 * @param sy
	 *            starting position
	...
	/**
	 * Updates distance values for adjacent cells that are reachable if value can be
	 * reduced to current distance plus 1.
	 * 
	 * @param cells
	 * @param currentX
	 ...
	 * @return direction for a neighbor that has been updated or null if there is
	 *         none
	 */
	...
	public int[] computeDistances(Cells cells) {
		...
		// figure out which position is the furthest on the border to find an exit
		// position
