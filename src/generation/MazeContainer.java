...
public class MazeContainer implements MazeConfiguration {
	...
	private BSPNode rootnode; // a binary tree type search data structure to quickly locate a subset of
								// segments
	// a segment is a continuous sequence of walls in vertical or horizontal
	// direction
	// a subset of segments need to be quickly identified for drawing
	// the BSP tree partitions the set of all segments and provides a binary search
	// tree for the partitions
	private int[] start;
	...
	/**
	 * Gives the cells. Warning, returns direct access to internal field.
	 * 
	...
	/**
	 * Gives the distance. Warning, returns direct access to internal field.
	 * 
	...
	/**
	 * Gives the rootnode for the tree of BSPnodes. Warning, returns direct access
	 * to internal field.
	 * 
	 * @return the rootnode
	 */
	...
	/**
	 * Gives a (x',y') neighbor for given (x,y) that is closer to exit if it exists.
	 * 
	 * @param x
	 *            is on the horizontal axis, 0 <= x < width
	 * @param y
	...
	public int[] getNeighborCloserToExit(int x, int y) {
		...
		if (getDistanceToExit(x, y) <= dnext) {
			System.out.println(
					"ERROR: MazeContainer.getNeighborCloserToExit cannot identify direction towards solution: stuck at: "
							+ x + ", " + y);
	...
	/**
	 * Provides the (x,y) starting position. The starting position is typically
	 * chosen to by furthest away from the exit.
	 * 
	 * @return the start
	 */
