...
/**
 * This class has the responsibility to obtain the tree of BSP nodes for a given
 * maze.
 * 
 * This code is refactored code from MazeBuilder.java by Paul Falstad,
...
public class BSPBuilder {
	...
	// colchange: reason for randomization unclear, used to determine color of
	// segment
	private final int expectedPartiters; // comes from Constants partct array, entry chosen according to skill level
	// only usage is in updateProgressBar to estimate progress made in the BSP tree
	// construction
	 ...
	 */
	public BSPBuilder(Order order, Distance dists, Cells cells, int width, int height, int colchange,
	...
	/**
	 * Method generates the nodes. In every node, it has two section, left and
	 * right. It chooses the segment which has the minimum grade value and then
	 * split this node into two nodes through this segment. If all the segments in
	 * one node are partitioned, it will stop to split.
	 * 
	 * @param sl
	...
	private BSPNode genNodes(ArrayList<Seg> sl) throws InterruptedException {
		// if there is no segment with a partition bit set to false, there is nothing
		// else to do and we are at a leaf node
		if (countNonPartitions(sl) == 0)
			return new BSPLeaf(sl);
		// from the ones that have a partition bit set to false, pick a candidate with a
		// low grade
		...
		for (int i = 0; i != sl.size(); i++) {
			...
			if (getSign(dot1) != getSign(dot2)) {
				if (dot1 == 0)
					...
					Seg sps1 = new Seg(se.getStartPositionX(), se.getStartPositionY(), spx - se.getStartPositionX(),
							spy - se.getStartPositionY(), se.getDistance(), colchange);
	...
	/**
	 * Counts how many elements in the segment vector have their partition bit set
	 * to false
	 * 
	 * @param sl
	 *            all segments
	 ...
	 * @param sl
	 *            vector of segments
	 * @return Segment that is best candidate according to grade partition (smallest
	 *         grade)
	 * @throws InterruptedException
	 */
	private Seg findPartitionCandidate(ArrayList<Seg> sl) throws InterruptedException {
		Seg result = null;
		int bestgrade = 5000; // used to compute the minimum of all observed grade values, set to some high
								// initial value
		final int maxtries = 50; // constant, only used to determine skip
		// consider a subset of segments proportional to the number of tries, here 50,
		// seems to randomize the access a bit
	...
	private void updateProgressBar(int partiters) throws InterruptedException {
		// During maze generation, the most time consuming part needs to occasionally
	...
	/**
	 * Set the partition bit to true for segments on the border and where the
	 * direction is 0
	 * 
	 * @param sl
	 */
	private void setPartitionBitForCertainSegments(ArrayList<Seg> sl) {
		// TODO: check if seg just works with width and height or needs map_unit
		// adjustment
		// System.out.println("set Partition bit in BSP builder with scaled width and
		// height values");
		for (Seg se : sl) {
	...
	/**
	 * Identifies segments of continuous walls on the maze and fills the segment
	 * list
	 * 
	...
	private ArrayList<Seg> generateSegments() {
		...
		// starting positions for segments seem to be chosen such that segments
		// represent top or left walls
		return sl;
	}
	...
	private void generateSegmentsForVerticalWalls(ArrayList<Seg> sl) {
		...
		for (x = 0; x != width; x++) {
			...
			while (y < height) {
				...
				// create segment with (x,starty) being being the actual start position of the
				// segment, y-starty being the positive length
				sl.add(new Seg(x * Constants.MAP_UNIT, starty * Constants.MAP_UNIT, 0,
						(y - starty) * Constants.MAP_UNIT, dists.getDistance(x, starty), colchange));
			}
			y = 0;
			while (y < height) {
				// find the beginning of a segment
				...
				// create segment with (x+1,y) being being one off in both directions from the
				// last cell in this segment, starty-y being the negative length
				// since we are looking at right walls, one off in the right direction (x+1) are
				// then cells that have this segment on its left hand side
				// for some reason the end position is used as a starting position and therefore
				// the length & direction is inverse
				sl.add(new Seg((x + 1) * Constants.MAP_UNIT, y * Constants.MAP_UNIT, 0,
						(starty - y) * Constants.MAP_UNIT, dists.getDistance(x, starty), colchange));
			}
		}
	}
	...
	private void generateSegmentsForVerticalWallsNew(ArrayList<Seg> sl) {
		...
		for (x = 0; x < width; x++) {
			...
			while (it.hasNext()) {
				...
				int starty = cur[0];
				y = cur[1];
				// create segment with (x,starty) being the actual start position of the
				// segment,
	...
	private void generateSegmentForHorizontalWalls(ArrayList<Seg> sl) {
		...
		for (y = 0; y != height; y++) {
			...
			while (x < width) {
				...
				// create segment with (x,y) being the end positions, startx-x being the
				// negative length
				// note the (x,y) is not part of the segment
				sl.add(new Seg(x * Constants.MAP_UNIT, y * Constants.MAP_UNIT, (startx - x) * Constants.MAP_UNIT, 0,
						dists.getDistance(startx, y), colchange));
			}
			...
			while (x < width) {
				...
				// create segment with (startx,y+1) being one below the start position, x-startx
				// being the positive length
	...
	/**
	 * Method called in genNodes to determine the minimum of all such grades. The
	 * method is static, i.e. it does not update internal attributes and just
	...
	public BSPNode generateBSPNodes() throws InterruptedException {
		// determine segments, i.e. walls over multiple cells in a vertical or
		// horizontal direction
		ArrayList<Seg> seglist = generateSegments();

		setPartitionBitForCertainSegments(seglist); // partition bit true means that those are not considered any
													// further for node generation

		// TODO: check why this is done. It creates a top wall on position (0,0). This
		// may even corrupt a maze and block its exit!
