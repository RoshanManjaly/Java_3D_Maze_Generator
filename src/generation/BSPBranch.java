...
/**
 * BSPNodes are used to build a binary tree, where internal nodes keep track of
 * lower and upper bounds of (x,y) coordinates. Leaf nodes carry a list of
 * segments. Branch nodes are internal nodes of the tree. A BSP tree is a data
 * structure to search for a set of segments to put on display in the
 * FirstPersonDrawer.
 * 
...
public class BSPBranch extends BSPNode {
	...
	/**
	 * Constructor with values for all internal fields Upper and lower bound values
	 * are obtained from the given left and right branches, so those trees must be
	 * carry valid values.
	 * 
	 * @param px
	 *            x coordinate
	 ...
	 *            y direction
	 * @param left
	 *            child, assumes bounds are valid
	 * @param right
	 *            child, assumes bounds are valid
	 */
	 ...
	 * 
	 * The method recursively stores BSP nodes for left and right children. Note
	 * that the numbering schemes needs to match with the MazeFileReader class.
	 * 
	 * @param n
	...
	public int store(Document doc, Element mazeXML, int number) {
		...
		// it is important that the recursion on the left branch updates the number
		// value
