...
public class BSPLeaf extends BSPNode {
	...
	/**
	 * Constructor stores reference to given list of segments and updates bounds
	 * 
	 * @param sl
	 *            is a list of segments, can not be empty
	 */
	...
	/**
	 * Update bounds based on min and max values seen in start and end positions of
	 * a given list of segments
	 * 
	 * @param sl
	 */
	private void updateBounds(ArrayList<Seg> sl) {
		/*
		 * setLowerBoundX(1000000); // TODO: poor programming, supposed to be largest
		 * possible integer setUpperBoundX(-1000000); // TODO: poor programming,
		 * supposed to be smallest possible integer setLowerBoundY(1000000); // TODO:
		 * poor programming, supposed to be largest possible integer
		 * setUpperBoundY(-1000000); // TODO: poor programming, supposed to be smallest
		 * possible integer for (int i = 0; i != sl.size(); i++) { Seg se =
		 * (Seg)slist.get(i); fix_bounds(se.getStartPositionX(),
		 * se.getStartPositionY()); fix_bounds(se.getEndPositionX(),
		 * se.getEndPositionY()); }
		 */
	...
	/**
	 * Store the content of a leaf node, in particular its list of segments. All
	 * entries carry the number of the node as an index and each segment has an
	 * additional second index for the segment number.
	 * 
	 * @param doc
	 ...
	 * @param number
	 *            is an index number for this node in the XML format
