...
/**
 * Represents absolute directions as for a map to match with the orientation on
 * the screen, i.e. North is no the top of the screen. In addition to limit the
 * set of possible values, this enum supports operations such as rotateClockwise
 * and getOppositeDirection. Translation methods between cardinal directions and
 * dx,dy pairs are supported to facilitate its integration in current code base.
 * 
 * Current mapping between cardinal directions and (dx,dy) east = (1,0) south =
 * (0,1) west = (-1,0) north = (0,-1)
 * 
 * Status: March 2016, coded, working but north/south upside down in graphics
 * Inconsistency: Cells.java: (0,0) at top-left corner, y coordinate increases
 * downwards MapDrawer: (0,0) at bottom-left corner, y coordinate increases
 * upwards
 * 
 * @author pk
 *
...
public enum CardinalDirection {
	...
	/**
	 * Gives the direction that results from a 90 degree clockwise rotation applied
	...
	/**
	 * This is a new method to help implement the BasicRobot class. Gives the
	 * direction that results from a 90 degree counter-clockwise rotation applied to
	...
	static public CardinalDirection getDirection(int dx, int dy) {
		/*
		 * Compare with Cells.java for consistency Directions: right=east, down=south,
		 * left=west, up=north public static int[] DIRS_X = { 1, 0, -1, 0 }; public
		 * static int[] DIRS_Y = { 0, 1, 0, -1 };
		 */
		// System.out.println("CardinalDirection.getDirection: Warning: check
		// consistency with direction on screen") ;
		switch (dx) {
		case -1: // must by (-1,0)
			return CardinalDirection.West;
		case 0:
