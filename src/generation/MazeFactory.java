...
/**
 * This class encapsulates how a maze is generated. It takes orders to produce a
 * maze, delegates it to the matching maze builder that computes and delivers
 * the maze. This class operates the worker thread to do the computation in the
 * background. The maze builder classes contribute a run method to execute on
 * the worker thread.
 */
public class MazeFactory implements Factory {
	// generation can be deterministic, i.e. same maze is generated each time for a
	// given size
	...
	public boolean order(Order order) {
		...
		default:
			System.out.println(
					"MazeFactory.order: missing implementation for requested algorithm: " + order.getBuilder());
			return false;
		}
		return true;
	}
	...
	public void waitTillDelivered() {
		if (null != buildThread) {
			try {
				...
				System.out.println(
						"MazeBuilder.wailTillDelivered: join synchronization with builder thread lead to an exception");
			}
		} else {
