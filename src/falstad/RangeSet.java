...
/**
 * Class stores a set of ranges, i.e., intervals. Its single current user is the
 * MazeController class.
 * 
 * Note: remove and intersect methods operate on a set of elements, however
 * there is no method to add more than a single element to the set as the set
 * method removes all previous elements from the set. This only way to increase
 * the cardinality is to split an existing interval into two by way of the
 * remove method.
 * 
...
public class RangeSet {
	 ...
	 * 
	 * @param mn
	 *            minimum value for new element
	 * @param mx
	 *            maximum value for new element
	 */
	...
	/**
	 * Removes interval [fx,tx] from existing set such that none of its elements
	 * intersects with it anymore. Existing intervals are reduced if they intersect,
	 * split into two or fully removed if they are contained in [fx,tx]
	 * 
	 * @param fx
	...
	/**
	 * Tells if there is at least one interval that intersects with [p.x,p.y]. It
	 * modifies attributes of p in case of an intersection. A point is used as a
	 * quick hack to communicate two integers and being able to return modified
	 * values in a boolean method.
	 * 
	 * @param p
	 *            describes interval [p.x,p.y], its entries are modified to
	 *            represent the intersection if there is one
	 * @return true if there is an intersection, false otherwise
	 */

	// POINT IS MISUSED HERE REPLACE IT WITH SOMETHING ELSE!!!!
