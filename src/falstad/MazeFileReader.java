...
/**
 * This class provides functionality to read the content of a Maze object from a
 * file. File format is XML and produced with MazeFileWriter. The class is a
 * simple wrapper to make all fields of a Maze object accessible such that with
 * the help of this class it is straightforward to instantiate a
 * MazeConfiguration object.
 * 
 *
 */
public class MazeFileReader {
	...
	/**
	 * Constructor reads maze data from given file. The file format is an XML format
	 * produced by the MazeFileWriter class.
	 * 
	 * @param filename
	 *            with data of a Maze object
	 */
	...
	/**
	 * Method provides main functionality to read all attributes of maze object from
	 * the given file
	 * 
	 * @param filename
		...
		try {
			...
			for (int temp = 0; temp < nList.getLength(); temp++) {
				...
				if (nNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					number = 0; // field used as an index, that is shared an updated across recursive
								// readBSPNode calls
					root = readBSPNode(eElement);
				}
			}
		} catch (Exception e) { // : implement serious error handling
			e.printStackTrace();
		}

	}

	// shared index number for nodes in the tree of BSPNodes, used as an additional
	// return value for recursive calls
	// the sequence of values is increasing, when switching from a left branch to a
	// right branch in a preorder treetraversal
	 ...
	 * 
	 * The method recursively explores the left and right branches and builds a
	 * complete tree.
	 * 
	 * @param eElement
	 *            element to read data from
	...
	private BSPNode readBSPNode(Element eElement) {
		...
		/*
		 * unused, as these values are recalculated in the BSPnode constructor int
		 * xlLoad = getElementIntValue("xlBSPNode_"+number, eElement); int ylLoad =
		 * getElementIntValue("ylBSPNode_"+number, eElement); int xuLoad =
		 * getElementIntValue("xuBSPNode_"+number, eElement); int yuLoad =
		 * getElementIntValue("yuBSPNode_"+number, eElement);
		 */
		int mynumber = number; // keep track of own node number, as that the shared attribute number gets
								// manipulated in recursive method calls
		...
		if (isleafLoad) {
			...
			// note xl, yl, xu and yu are computed from the segments within the leaf
			// constructor
			...
			// read left branch before right branch, increment index number for next node to
			// visit
			number++;
			...
			// other fields of BSBNode class need not be set, computed in constructor from
			// branches
	 ...
	 * @param number
	 *            suffix with index number of BSPNode
	 * @param i
	 *            suffix with index of segment
	...
	private static Seg readSegment(Element eElement, int number, int i) {
		...
		int cc = 0; // use this as a dummy for the constructor, the correct color is explicitly set
					// below
		Seg result = new Seg(x, y, dx, dy, dist, cc);
		// ADDRESS THIS , from seg.setColor method
	...
	/**
	 * Reads data for a two-dimensional array of distance values. Requires that
	 * fields width and height have been set.
	 * 
	 * @param eElement
	 *            to read data from
	 */
	...
	/**
	 * Reads data for a cells object that contains values for walls. Requires that
	 * fields width and height have been set.
	 * 
	 * @param eElement
	 *            to read data from
	...
	public static int getElementIntValue(String string, Element parent) {
		return Integer.parseInt(getElementStringValue(string, parent));
	}
	...
	public static boolean getElementBooleanValue(String string, Element parent) {
		return Boolean.valueOf(getElementStringValue(string, parent)).booleanValue();
	}
	...
	/////////////////////////////////// /////////////////////////////////////////
	// : change these into equals and compare methods for the corresponding Maze
	/////////////////////////////////// and BSPNode classes
	// : create junit test class that is a subclass of MazeFileReader that takes
	/////////////////////////////////// this code and performs unit tests
	/**
	 ...
	 * @param py
	 */
	public void compare(int mazew, int mazeh, int rooms2, int expected_partiters2, BSPNode root2, Cells mazecells,
	...
	private static void compareBSPNodes(BSPNode root, BSPNode root2) {
		...
		if (BSPLeaf.class == root.getClass()) {
			if (BSPLeaf.class != root2.getClass())
				System.out.println(
						"MazeFileReader.compareBSPNodes: type of nodes mismatch, root node has leaf, other node as branch");
