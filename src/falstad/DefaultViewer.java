...
public class DefaultViewer implements Viewer {
	...
	//@Override
	// MOVE THIS METHOD TO MAZE PANEL
	...
	public void redraw(Graphics gc, StateGUI state, int px, int py,
			int view_dx, int view_dy, int walk_step, int view_offset, RangeSet rset, int ang) {
				redraw(gc, state, px, py, view_dx, view_dy, walk_step, view_offset, ang);
			}
	...
	public void redraw(Graphics gc, StateGUI state, int px, int py, int view_dx, int view_dy, int walk_step, int view_offset, int ang) {
		dbg("redraw") ;
	}
}
