...
/**
 * Class handles the user interaction. It implements a state-dependent behavior
 * that controls the display and reacts to key board input from a user. At this
 * point user keyboard input is first dealt with a key listener
 * (SimpleKeyListener) and then handed over to a MazeController object by way of
 * the keyDown method.
 *
...
public class MazeController implements Order {
	...
	// This class acts as the controller that gets user input and operates on the
	// model.
	// A MazeConfiguration acts as the model and this class has a reference to it.
	// changed mazeConfig from private to public
	...
	// state keeps track of the current GUI state, one of
	// STATE_TITLE,...,STATE_FINISH, mainly used in redraw()
	...
	// STATE_PLAY is the main state where the user can navigate through the maze in
	// a first person view
	...
	// map_mode is toggled by user keyboard input, causes a call to draw_map during
	// play mode
	...
	private Cells seencells; // a matrix with cells to memorize which cells are visible from the current
								// point of view
	// the FirstPersonDrawer obtains this information and the MapDrawer uses it for
	...
	/**
	 * Constructor Default setting for maze generating algorithm is DFS.
	 */
	 ...
	 */
	public MazeController(Order.Builder builder) // builder was there before
	{
		super();
		setBuilder(builder);
		panel = new MazePanel();
	...
	// ------added constructor for driver--------
	public MazeController(ManualDriver driver) {
		super();
	...
	/**
	 * Method to initialize internal attributes. Called separately from the
	 * constructor.
	 */
	public void init() {
	...
	private void switchToGeneratingScreen() {
		assert state == StateGUI.STATE_TITLE : "MazeController.switchToGeneratingScreen: unexpected current state "
				+ state;
	...
	/**
	 * Switches to playing state, registers appropriate views, updates screen. This
	 * transition is only possible from the generating screen.
	 */
	private void switchToPlayingScreen() {
		assert state == StateGUI.STATE_GENERATING : "MazeController.switchToPlayingScreen: unexpected current state "
				+ state;
		...
		addView(new MapDrawer(Constants.VIEW_WIDTH, Constants.VIEW_HEIGHT, Constants.MAP_UNIT, Constants.STEP_SIZE,
				seencells, 10, this));
	...
	/**
	 * Switches to title screen, possibly canceling maze generation. This transition
	 * is possible from several screens.
	 */
	...
	/**
	 * Switches to title screen, possibly canceling maze generation. This transition
	 * is only possible from the playing screen by making a forward or backward move
	 * through the exit outside of the maze.
	 */
	protected void switchToFinishScreen() { // WHERE BATTERY STUFF GOES
	...
	private void logPosition() {
		if (!deepdebug)
			return;
		dbg("x=" + viewx / Constants.MAP_UNIT + " (" + viewx + ") y=" + viewy / Constants.MAP_UNIT + " (" + viewy
				+ ") ang=" + angle + " dx=" + dx + " dy=" + dy + " " + viewdx + " " + viewdy);
	}
	...
	/**
	 * Redraw and wait, used to obtain a smooth appearance for rotate and move
	 * operations
	 */
	 ...
	 * 
	 * @param dir,
	 *            only possible values are 1 (forward) and -1 (backward)
	 */
	...
	public enum UserInput {
		ReturnToTitle, Start, Up, Down, Left, Right, Jump, ToggleLocalMap, ToggleFullMap, ToggleSolution, ZoomIn, ZoomOut
	};
	 ...
	 */
	public boolean keyDown(UserInput key, int value) { // LOOK AT THIS METHOD
		...
		// implemented as a little automaton that switches state and performs necessary
		// actions
		switch (state) {
		case STATE_TITLE:
			...
			break;
		// if we are currently generating a maze, recognize interrupt signal (ESCAPE
		// key)
		...
		case STATE_PLAY:
			...
			operateGameInPlayingState(key); // FOR NOW

			/*
			 * if (key == UserInput.Up) { driver.moveForward(); } if (key == UserInput.Down)
			 * { driver.moveBackward(); } if (key == UserInput.Left) { driver.rotateLeft();
			 * } if (key == UserInput.Right) { driver.rotateRight(); } else {
	...
	public void operateGameInPlayingState(UserInput key) {
		...
		case ZoomIn: // zoom into map
			...
			panel.notifyViewerRedraw(this); // seems useless but it is necessary to make the screen update
			break;
	...
	/**
	 * Sets the internal state of the game state. Method checks if state transition
	 * is as expected.
	 * 
	 * @param newState
	 *            the state to set
	...
	/**
	 * Allows external increase to percentage in generating mode. Internal value is
	 * only update if it exceeds the last value and is less or equal 100
	 * 
	 * @param percentage
	 *            gives the new percentage on a range [0,100]
