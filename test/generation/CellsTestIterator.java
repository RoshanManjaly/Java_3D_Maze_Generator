...
public class CellsTestIterator {
	 ...
	 * <p>
	 * If there are no walls, iterator should provide no sequences. Test uses fixed
	 * width and height but iterates through all positions on the maze and exercises
	 * all directions.
	 */
	@Test
	 ...
	 * Method under test: iterator
	 * <p>
	 * If all walls are up, iterator should return sequences of length 1 because we
	 * hit a crossing wall after each step. Test uses fixed width and height but
	 * iterates through all positions on the maze and exercises all directions.
	 */
	@Test
	 ...
	 * 
	 * @param cells
	 *            that carry the walls
	 * @param cdBlocked
	 *            the direction we could see a blocking wall (perpendicular)
	 * @param limit
	 *            the max value for the end position for the y coordinate
	 * @param cd
	 *            the direction we expect to have a wall
	 * @param x
	 *            coordinate, the column in cells
	 * @param seq
	 *            gives two y coordinate values, start and end
	 */
	private void isSequenceOfVerticalWalls(Cells cells, CardinalDirection cdBlocked, int limit, CardinalDirection cd,
			int x, int[] seq) {
		...
		assertTrue(seq[1] == limit || cells.hasNoWall(x, seq[1], cd) || (cells.hasWall(x, seq[1], cdBlocked)
	 ...
	 *            the direction we expect to have a wall
	 * @param y
	 *            coordinate, the row in cells
	 * @param seq
	 *            gives two x coordinate values, start and end
	 */
	private void isSequenceOfHorizontalWalls(Cells cells, CardinalDirection cdBlocked, int limit, CardinalDirection cd,
			int y, int[] seq) {
		// for east, there should be a wall at (x,start)
		...
		// note: adjusted by picked opposite direction for seq[1] for cdBlocked
		// as walls are symmetric (represented in both adjacent cells)
		assertTrue(seq[1] == limit || cells.hasNoWall(seq[1], y, cd) || (cells.hasWall(seq[1], y, cdBlocked)
	 ...
	 * Method under test: iterator
	 * <p>
	 * Test starts with a cell where all walls are up and then deletes walls in a
	 * regular manner such that all sequence should be at the same fixed length. The
	 * gap between sequences is always of same length as well. Test uses fixed width
	 * and height but iterates through all positions on the maze and exercises all
	 * directions.
	 */
	@Test
	...
	/**
	 * All columns treated the same, start with a sequence of walls, then a gap,
	 * then a sequence and so forth.
	 * 
	 * @param cells
	 * @param width
	 * @param height
	 ...
	 * @param gapLength
	 */
	private void punchHolesEast(final Cells cells, final int width, final int height, final int seqLength,
			final int gapLength) {
		int c = 0;
		boolean gap = false;
